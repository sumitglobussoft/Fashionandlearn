...
if (!defined('__CLASS_HTML2PDF__')) {
    ...
    class html2pdf_HTML2PDF {
        ...
        /**
         * Clone to create a sub HTML2PDF fromhtml2pdf_HTML2PDF::$_subobj
         *
         * @access public
         */
        ...
        protected function _cleanUp() {
            html2pdf_HTML2PDF::$_subobj = null;
            html2pdf_HTML2PDF::$_tables = array();
        }
...
' . $content . '
        ...
        /**
         * preparehtml2pdf_HTML2PDF::$_subobj (used for create the sub HTML2PDF objects
         *
         * @access protected
         */
        protected function _prepareSubObj() {
            ...
            // create the sub object
            html2pdf_HTML2PDF::$_subobj = new html2pdf_HTML2PDF(
            ...
            // init
            html2pdf_HTML2PDF::$_subobj->setTestTdInOnePage($this->_testTdInOnepage);
            html2pdf_HTML2PDF::$_subobj->setTestIsImage($this->_testIsImage);
            html2pdf_HTML2PDF::$_subobj->setTestIsDeprecated($this->_testIsDeprecated);
            html2pdf_HTML2PDF::$_subobj->setDefaultFont($this->_defaultFont);
            html2pdf_HTML2PDF::$_subobj->parsingCss->css = &$this->parsingCss->css;
            html2pdf_HTML2PDF::$_subobj->parsingCss->cssKeys = &$this->parsingCss->cssKeys;

            // clone font from the original PDF
            html2pdf_HTML2PDF::$_subobj->pdf->cloneFontFrom($this->pdf);

            // remove the link to the parent
            html2pdf_HTML2PDF::$_subobj->parsingCss->setPdfParent($pdf);
        }
        ...
        protected function _createSubHTML(&$subHtml, $cellmargin = 0) {
            ...
            if (html2pdf_HTML2PDF::$_subobj === null) {
                $this->_prepareSubObj();
            }
            ...
            html2pdf_HTML2PDF::$_subobj->pdf->getPage();

            // clone the sub oject
            $subHtml = clone html2pdf_HTML2PDF::$_subobj;
        ...
        protected function _tag_open_THEAD($param) {
            ...
            if ($this->_subPart) {
                html2pdf_HTML2PDF::$_tables[$param['num']]['thead']['tr'][0] = html2pdf_HTML2PDF::$_tables[$param['num']]['tr_curr'];
                html2pdf_HTML2PDF::$_tables[$param['num']]['thead']['code'] = array();
                for ($pos = $this->_tempPos; $pos < count($this->parsingHtml->code); $pos++) {
                    ...
                    html2pdf_HTML2PDF::$_tables[$param['num']]['thead']['code'][] = $action;
                ...
                html2pdf_HTML2PDF::$_tables[$param['num']]['tr_curr']+= count(HTML2PDF::$_tables[$param['num']]['thead']['tr']);
            }
        ...
        protected function _tag_close_THEAD($param) {
            ...
            if ($this->_subPart) {
                $min = html2pdf_HTML2PDF::$_tables[$param['num']]['thead']['tr'][0];
                $max = html2pdf_HTML2PDF::$_tables[$param['num']]['tr_curr'] - 1;
                html2pdf_HTML2PDF::$_tables[$param['num']]['thead']['tr'] = range($min, $max);
            }
        ...
        protected function _tag_open_TFOOT($param) {
            ...
            if ($this->_subPart) {
                html2pdf_HTML2PDF::$_tables[$param['num']]['tfoot']['tr'][0] = html2pdf_HTML2PDF::$_tables[$param['num']]['tr_curr'];
                html2pdf_HTML2PDF::$_tables[$param['num']]['tfoot']['code'] = array();
                for ($pos = $this->_tempPos; $pos < count($this->parsingHtml->code); $pos++) {
                    $action = $this->parsingHtml->code[$pos];
                    if (strtolower($action['name']) == 'tfoot')
                        $action['name'] = 'tfoot_sub';
                    html2pdf_HTML2PDF::$_tables[$param['num']]['tfoot']['code'][] = $action;
                ...
                $level = $this->parsingHtml->getLevel($this->_parsePos);
                $this->_parsePos+= count($level);
                html2pdf_HTML2PDF::$_tables[$param['num']]['tr_curr']+= count(HTML2PDF::$_tables[$param['num']]['tfoot']['tr']);
            }
        ...
        protected function _tag_close_TFOOT($param) {
            ...
            if ($this->_subPart) {
                $min = html2pdf_HTML2PDF::$_tables[$param['num']]['tfoot']['tr'][0];
                $max = html2pdf_HTML2PDF::$_tables[$param['num']]['tr_curr'] - 1;
                html2pdf_HTML2PDF::$_tables[$param['num']]['tfoot']['tr'] = range($min, $max);
            }
        ...
        protected function _tag_open_TABLE($param, $other = 'table') {
            ...
            if ($this->_subPart) {
                ...
                html2pdf_HTML2PDF::$_tables[$param['num']] = array();
                html2pdf_HTML2PDF::$_tables[$param['num']]['border'] = isset($param['border']) ? $this->parsingCss->readBorder($param['border']) : null;
                html2pdf_HTML2PDF::$_tables[$param['num']]['cellpadding'] = $this->parsingCss->ConvertToMM(isset($param['cellpadding']) ? $param['cellpadding'] : '1px');
                html2pdf_HTML2PDF::$_tables[$param['num']]['cellspacing'] = $this->parsingCss->ConvertToMM(isset($param['cellspacing']) ? $param['cellspacing'] : '2px');
                html2pdf_HTML2PDF::$_tables[$param['num']]['cases'] = array();          // properties of each TR/TD
                html2pdf_HTML2PDF::$_tables[$param['num']]['corr'] = array();          // link between TR/TD and colspan/rowspan
                html2pdf_HTML2PDF::$_tables[$param['num']]['corr_x'] = 0;                // position in 'cases'
                html2pdf_HTML2PDF::$_tables[$param['num']]['corr_y'] = 0;                // position in 'cases'
                html2pdf_HTML2PDF::$_tables[$param['num']]['td_curr'] = 0;                // current column
                html2pdf_HTML2PDF::$_tables[$param['num']]['tr_curr'] = 0;                // current row
                html2pdf_HTML2PDF::$_tables[$param['num']]['curr_x'] = $this->pdf->getX();
                html2pdf_HTML2PDF::$_tables[$param['num']]['curr_y'] = $this->pdf->getY();
                html2pdf_HTML2PDF::$_tables[$param['num']]['width'] = 0;                // global width
                html2pdf_HTML2PDF::$_tables[$param['num']]['height'] = 0;                // global height
                html2pdf_HTML2PDF::$_tables[$param['num']]['align'] = $alignObject;
                html2pdf_HTML2PDF::$_tables[$param['num']]['marge'] = array();
                html2pdf_HTML2PDF::$_tables[$param['num']]['marge']['t'] = $this->parsingCss->value['padding']['t'] + $this->parsingCss->value['border']['t']['width'] + html2pdf_HTML2PDF::$_tables[$param['num']]['cellspacing'] * 0.5;
                html2pdf_HTML2PDF::$_tables[$param['num']]['marge']['r'] = $this->parsingCss->value['padding']['r'] + $this->parsingCss->value['border']['r']['width'] + html2pdf_HTML2PDF::$_tables[$param['num']]['cellspacing'] * 0.5;
                html2pdf_HTML2PDF::$_tables[$param['num']]['marge']['b'] = $this->parsingCss->value['padding']['b'] + $this->parsingCss->value['border']['b']['width'] + html2pdf_HTML2PDF::$_tables[$param['num']]['cellspacing'] * 0.5;
                html2pdf_HTML2PDF::$_tables[$param['num']]['marge']['l'] = $this->parsingCss->value['padding']['l'] + $this->parsingCss->value['border']['l']['width'] + html2pdf_HTML2PDF::$_tables[$param['num']]['cellspacing'] * 0.5;
                html2pdf_HTML2PDF::$_tables[$param['num']]['page'] = 0;                // number of pages
                html2pdf_HTML2PDF::$_tables[$param['num']]['new_page'] = true;             // flag : new page for the current TR
                html2pdf_HTML2PDF::$_tables[$param['num']]['style_value'] = null;             // CSS style of the table
                html2pdf_HTML2PDF::$_tables[$param['num']]['thead'] = array();          // properties on the thead
                html2pdf_HTML2PDF::$_tables[$param['num']]['tfoot'] = array();          // properties on the tfoot
                html2pdf_HTML2PDF::$_tables[$param['num']]['thead']['tr'] = array();          // list of the TRs in the thead
                html2pdf_HTML2PDF::$_tables[$param['num']]['tfoot']['tr'] = array();          // list of the TRs in the tfoot
                html2pdf_HTML2PDF::$_tables[$param['num']]['thead']['height'] = 0;             // thead height
                html2pdf_HTML2PDF::$_tables[$param['num']]['tfoot']['height'] = 0;             // tfoot height
                html2pdf_HTML2PDF::$_tables[$param['num']]['thead']['code'] = array();            // HTML content of the thead
                html2pdf_HTML2PDF::$_tables[$param['num']]['tfoot']['code'] = array();            // HTML content of the tfoot
                html2pdf_HTML2PDF::$_tables[$param['num']]['cols'] = array();              // properties of the COLs

                $this->_saveMargin($this->pdf->getlMargin(), $this->pdf->gettMargin(), $this->pdf->getrMargin());

                $this->parsingCss->value['width']-=html2pdf_HTML2PDF::$_tables[$param['num']]['marge']['l'] + html2pdf_HTML2PDF::$_tables[$param['num']]['marge']['r'];
            } else {
                // we start from the first page and the first page of the table
                html2pdf_HTML2PDF::$_tables[$param['num']]['page'] = 0;
                html2pdf_HTML2PDF::$_tables[$param['num']]['td_curr'] = 0;
                html2pdf_HTML2PDF::$_tables[$param['num']]['tr_curr'] = 0;
                html2pdf_HTML2PDF::$_tables[$param['num']]['td_x'] = html2pdf_HTML2PDF::$_tables[$param['num']]['marge']['l'] + html2pdf_HTML2PDF::$_tables[$param['num']]['curr_x'];
                html2pdf_HTML2PDF::$_tables[$param['num']]['td_y'] = html2pdf_HTML2PDF::$_tables[$param['num']]['marge']['t'] + html2pdf_HTML2PDF::$_tables[$param['num']]['curr_y'];

                // draw the borders/background of the first page/part of the table
                $this->_drawRectangle(
                        html2pdf_HTML2PDF::$_tables[$param['num']]['curr_x'], html2pdf_HTML2PDF::$_tables[$param['num']]['curr_y'], html2pdf_HTML2PDF::$_tables[$param['num']]['width'], isset(html2pdf_HTML2PDF::$_tables[$param['num']]['height'][0]) ? html2pdf_HTML2PDF::$_tables[$param['num']]['height'][0] : null, $this->parsingCss->value['border'], $this->parsingCss->value['padding'], 0, $this->parsingCss->value['background']
                );

                html2pdf_HTML2PDF::$_tables[$param['num']]['style_value'] = $this->parsingCss->value;
            }
        ...
        protected function _tag_close_TABLE($param) {
            ...
            if ($this->_subPart) {
                ...
                $this->_calculateTableCellSize(html2pdf_HTML2PDF::$_tables[$param['num']]['cases'], html2pdf_HTML2PDF::$_tables[$param['num']]['corr']);

                // calculate the height of the thead and the tfoot
                $lst = array('thead', 'tfoot');
                foreach ($lst as $mode) {
                    html2pdf_HTML2PDF::$_tables[$param['num']][$mode]['height'] = 0;
                    foreach (html2pdf_HTML2PDF::$_tables[$param['num']][$mode]['tr'] as $tr) {
                        // hauteur de la ligne tr
                        $h = 0;
                        for ($i = 0; $i < count(HTML2PDF::$_tables[$param['num']]['cases'][$tr]); $i++)
                            if (html2pdf_HTML2PDF::$_tables[$param['num']]['cases'][$tr][$i]['rowspan'] == 1)
                                $h = max($h, html2pdf_HTML2PDF::$_tables[$param['num']]['cases'][$tr][$i]['h']);
                        html2pdf_HTML2PDF::$_tables[$param['num']][$mode]['height']+= $h;
                    }
                }

                // calculate the width of the table
                html2pdf_HTML2PDF::$_tables[$param['num']]['width'] = html2pdf_HTML2PDF::$_tables[$param['num']]['marge']['l'] + html2pdf_HTML2PDF::$_tables[$param['num']]['marge']['r'];
                if (isset(html2pdf_HTML2PDF::$_tables[$param['num']]['cases'][0])) {
                    foreach (html2pdf_HTML2PDF::$_tables[$param['num']]['cases'][0] as $case) {
                        html2pdf_HTML2PDF::$_tables[$param['num']]['width']+= $case['w'];
                    }
                }
                ...
                $old = $this->parsingCss->getOldValues();
                $parentWidth = $old['width'] ? $old['width'] : $this->pdf->getW() - $this->pdf->getlMargin() - $this->pdf->getrMargin();
                $x = html2pdf_HTML2PDF::$_tables[$param['num']]['curr_x'];
                $w = html2pdf_HTML2PDF::$_tables[$param['num']]['width'];
                if ($parentWidth > $w) {
                    if (html2pdf_HTML2PDF::$_tables[$param['num']]['align'] == 'center')
                        $x = $x + ($parentWidth - $w) * 0.5;
                    else if (html2pdf_HTML2PDF::$_tables[$param['num']]['align'] == 'right')
                        $x = $x + $parentWidth - $w;

                    html2pdf_HTML2PDF::$_tables[$param['num']]['curr_x'] = $x;
                }

                // calculate the height of the table
                html2pdf_HTML2PDF::$_tables[$param['num']]['height'] = array();

                // minimum of the height because of margins, and of the thead and tfoot height
                $h0 = html2pdf_HTML2PDF::$_tables[$param['num']]['marge']['t'] + html2pdf_HTML2PDF::$_tables[$param['num']]['marge']['b'];
                $h0+=html2pdf_HTML2PDF::$_tables[$param['num']]['thead']['height'] + html2pdf_HTML2PDF::$_tables[$param['num']]['tfoot']['height'];

                // max height of the page
                $max = $this->pdf->getH() - $this->pdf->getbMargin();

                // current position on the page
                $y = html2pdf_HTML2PDF::$_tables[$param['num']]['curr_y'];
                $height = $h0;

                // we get the height of each line
                for ($k = 0; $k < count(html2pdf_HTML2PDF::$_tables[$param['num']]['cases']); $k++) {

                    // if it is a TR of the thead or of the tfoot => skip
                    if (in_array($k, html2pdf_HTML2PDF::$_tables[$param['num']]['thead']['tr']))
                        continue;
                    if (in_array($k, html2pdf_HTML2PDF::$_tables[$param['num']]['tfoot']['tr']))
                        continue;

                    // height of the line
                    $th = 0;
                    $h = 0;
                    for ($i = 0; $i < count(html2pdf_HTML2PDF::$_tables[$param['num']]['cases'][$k]); $i++) {
                        $h = max($h, html2pdf_HTML2PDF::$_tables[$param['num']]['cases'][$k][$i]['h']);

                        if (html2pdf_HTML2PDF::$_tables[$param['num']]['cases'][$k][$i]['rowspan'] == 1)
                            $th = max($th, html2pdf_HTML2PDF::$_tables[$param['num']]['cases'][$k][$i]['h']);
                    }
                    ...
                    if ($y + $h + $height > $max) {
                        if ($height == $h0)
                            $height = null;
                        html2pdf_HTML2PDF::$_tables[$param['num']]['height'][] = $height;
                        $height = $h0;
                        $y = $this->_margeTop;
                    }
                    $height+= $th;
                ...
                if (count(html2pdf_HTML2PDF::$_tables[$param['num']]['tfoot']['code'])) {
                    $tmpTR = html2pdf_HTML2PDF::$_tables[$param['num']]['tr_curr'];
                    $tmpTD = html2pdf_HTML2PDF::$_tables[$param['num']]['td_curr'];
                    $oldParsePos = $this->_parsePos;
                    $oldParseCode = $this->parsingHtml->code;

                    html2pdf_HTML2PDF::$_tables[$param['num']]['tr_curr'] = html2pdf_HTML2PDF::$_tables[$param['num']]['tfoot']['tr'][0];
                    html2pdf_HTML2PDF::$_tables[$param['num']]['td_curr'] = 0;
                    $this->_parsePos = 0;
                    $this->parsingHtml->code = html2pdf_HTML2PDF::$_tables[$param['num']]['tfoot']['code'];
                    ...
                    $this->_parsePos = $oldParsePos;
                    $this->parsingHtml->code = $oldParseCode;
                    html2pdf_HTML2PDF::$_tables[$param['num']]['tr_curr'] = $tmpTR;
                    html2pdf_HTML2PDF::$_tables[$param['num']]['td_curr'] = $tmpTD;
                }

                // get the positions of the end of the table
                $x = html2pdf_HTML2PDF::$_tables[$param['num']]['curr_x'] + html2pdf_HTML2PDF::$_tables[$param['num']]['width'];
                if (count(html2pdf_HTML2PDF::$_tables[$param['num']]['height']) > 1)
                    $y = $this->_margeTop + html2pdf_HTML2PDF::$_tables[$param['num']]['height'][count(html2pdf_HTML2PDF::$_tables[$param['num']]['height']) - 1];
                else if (count(html2pdf_HTML2PDF::$_tables[$param['num']]['height']) == 1)
                    $y = html2pdf_HTML2PDF::$_tables[$param['num']]['curr_y'] + html2pdf_HTML2PDF::$_tables[$param['num']]['height'][count(html2pdf_HTML2PDF::$_tables[$param['num']]['height']) - 1];
                else
                    $y = html2pdf_HTML2PDF::$_tables[$param['num']]['curr_y'];
        ...
        protected function _tag_open_COL($param) {
            ...
            for ($k = 0; $k < $span; $k++)
                html2pdf_HTML2PDF::$_tables[$param['num']]['cols'][] = $param;
        }
        ...
        protected function _tag_open_TR($param, $other = 'tr') {
            ...
            html2pdf_HTML2PDF::$_tables[$param['num']]['tr_curr'] ++;
            html2pdf_HTML2PDF::$_tables[$param['num']]['td_curr'] = 0;
            ...
            if (!$this->_subPart) {
                ...
                // Y after the row
                $ty = null;
                for ($ii = 0; $ii < count(html2pdf_HTML2PDF::$_tables[$param['num']]['cases'][html2pdf_HTML2PDF::$_tables[$param['num']]['tr_curr'] - 1]); $ii++) {
                    $ty = max($ty, html2pdf_HTML2PDF::$_tables[$param['num']]['cases'][html2pdf_HTML2PDF::$_tables[$param['num']]['tr_curr'] - 1][$ii]['h']);
                }

                // height of the tfoot
                $hfoot = html2pdf_HTML2PDF::$_tables[$param['num']]['tfoot']['height'];

                // if the line does not fit on the page => new page
                if (!$this->_isInTfoot && html2pdf_HTML2PDF::$_tables[$param['num']]['td_y'] + html2pdf_HTML2PDF::$_tables[$param['num']]['marge']['b'] + $ty + $hfoot > $this->pdf->getH() - $this->pdf->getbMargin()) {
                    ...
                    // new page
                    html2pdf_HTML2PDF::$_tables[$param['num']]['new_page'] = true;
                    $this->_setNewPage();

                    // new position
                    html2pdf_HTML2PDF::$_tables[$param['num']]['page'] ++;
                    html2pdf_HTML2PDF::$_tables[$param['num']]['curr_y'] = $this->pdf->getY();
                    html2pdf_HTML2PDF::$_tables[$param['num']]['td_y'] = html2pdf_HTML2PDF::$_tables[$param['num']]['curr_y'] + html2pdf_HTML2PDF::$_tables[$param['num']]['marge']['t'];

                    // if we have the height of the tbale on the page => draw borders and background
                    if (isset(html2pdf_HTML2PDF::$_tables[$param['num']]['height'][html2pdf_HTML2PDF::$_tables[$param['num']]['page']])) {
                        $old = $this->parsingCss->value;
                        $this->parsingCss->value = html2pdf_HTML2PDF::$_tables[$param['num']]['style_value'];

                        $this->_drawRectangle(
                                html2pdf_HTML2PDF::$_tables[$param['num']]['curr_x'], html2pdf_HTML2PDF::$_tables[$param['num']]['curr_y'], html2pdf_HTML2PDF::$_tables[$param['num']]['width'], html2pdf_HTML2PDF::$_tables[$param['num']]['height'][html2pdf_HTML2PDF::$_tables[$param['num']]['page']], $this->parsingCss->value['border'], $this->parsingCss->value['padding'], html2pdf_HTML2PDF::$_tables[$param['num']]['cellspacing'] * 0.5, $this->parsingCss->value['background']
                        );
                ...
                if (html2pdf_HTML2PDF::$_tables[$param['num']]['new_page'] && count(html2pdf_HTML2PDF::$_tables[$param['num']]['thead']['code'])) {
                    html2pdf_HTML2PDF::$_tables[$param['num']]['new_page'] = false;
                    $tmpTR = html2pdf_HTML2PDF::$_tables[$param['num']]['tr_curr'];
                    $tmpTD = html2pdf_HTML2PDF::$_tables[$param['num']]['td_curr'];
                    $oldParsePos = $this->_parsePos;
                    $oldParseCode = $this->parsingHtml->code;

                    html2pdf_HTML2PDF::$_tables[$param['num']]['tr_curr'] = html2pdf_HTML2PDF::$_tables[$param['num']]['thead']['tr'][0];
                    html2pdf_HTML2PDF::$_tables[$param['num']]['td_curr'] = 0;
                    $this->_parsePos = 0;
                    $this->parsingHtml->code = html2pdf_HTML2PDF::$_tables[$param['num']]['thead']['code'];
                ...
                // prepare it
                html2pdf_HTML2PDF::$_tables[$param['num']]['cases'][html2pdf_HTML2PDF::$_tables[$param['num']]['tr_curr'] - 1] = array();
                if (!isset(html2pdf_HTML2PDF::$_tables[$param['num']]['corr'][html2pdf_HTML2PDF::$_tables[$param['num']]['corr_y']]))
                    html2pdf_HTML2PDF::$_tables[$param['num']]['corr'][html2pdf_HTML2PDF::$_tables[$param['num']]['corr_y']] = array();

                html2pdf_HTML2PDF::$_tables[$param['num']]['corr_x'] = 0;
                while (isset(html2pdf_HTML2PDF::$_tables[$param['num']]['corr'][html2pdf_HTML2PDF::$_tables[$param['num']]['corr_y']][html2pdf_HTML2PDF::$_tables[$param['num']]['corr_x']]))
                    html2pdf_HTML2PDF::$_tables[$param['num']]['corr_x'] ++;
            }
        ...
        protected function _tag_close_TR($param) {
            ...
            if (!$this->_subPart) {
                ...
                $ty = null;
                for ($ii = 0; $ii < count(html2pdf_HTML2PDF::$_tables[$param['num']]['cases'][html2pdf_HTML2PDF::$_tables[$param['num']]['tr_curr'] - 1]); $ii++) {
                    if (html2pdf_HTML2PDF::$_tables[$param['num']]['cases'][html2pdf_HTML2PDF::$_tables[$param['num']]['tr_curr'] - 1][$ii]['rowspan'] == 1) {
                        $ty = html2pdf_HTML2PDF::$_tables[$param['num']]['cases'][html2pdf_HTML2PDF::$_tables[$param['num']]['tr_curr'] - 1][$ii]['h'];
                    }
                }

                // new position
                html2pdf_HTML2PDF::$_tables[$param['num']]['td_x'] = html2pdf_HTML2PDF::$_tables[$param['num']]['curr_x'] + html2pdf_HTML2PDF::$_tables[$param['num']]['marge']['l'];
                html2pdf_HTML2PDF::$_tables[$param['num']]['td_y']+= $ty;
                html2pdf_HTML2PDF::$_tables[$param['num']]['new_page'] = false;
            } else {
                html2pdf_HTML2PDF::$_tables[$param['num']]['corr_y'] ++;
            }
        ...
        protected function _tag_open_TD($param, $other = 'td') {
            ...
            $param['cellpadding'] = html2pdf_HTML2PDF::$_tables[$param['num']]['cellpadding'] . 'mm';
            $param['cellspacing'] = html2pdf_HTML2PDF::$_tables[$param['num']]['cellspacing'] . 'mm';
            ...
            $x = html2pdf_HTML2PDF::$_tables[$param['num']]['td_curr'];
            $y = html2pdf_HTML2PDF::$_tables[$param['num']]['tr_curr'] - 1;
            ...
            if (in_array($other, array('td', 'th'))) {
                // id of the column
                $numCol = isset(html2pdf_HTML2PDF::$_tables[$param['num']]['cases'][$y][$x]['Xr']) ? html2pdf_HTML2PDF::$_tables[$param['num']]['cases'][$y][$x]['Xr'] : html2pdf_HTML2PDF::$_tables[$param['num']]['corr_x'];

                // we get the properties of the COL tag, if exist
                if (isset(html2pdf_HTML2PDF::$_tables[$param['num']]['cols'][$numCol])) {

                    $colParam = html2pdf_HTML2PDF::$_tables[$param['num']]['cols'][$numCol];

                    // for colspans => we get all the neede widths
                    $colParam['style']['width'] = array();
                    for ($k = 0; $k < $colspan; $k++) {
                        if (isset(html2pdf_HTML2PDF::$_tables[$param['num']]['cols'][$numCol + $k]['style']['width'])) {
                            $colParam['style']['width'][] = html2pdf_HTML2PDF::$_tables[$param['num']]['cols'][$numCol + $k]['style']['width'];
                        }
                    }
            ...
            if (in_array($other, array('td', 'th'))) {
                ...
                if (html2pdf_HTML2PDF::$_tables[$param['num']]['border']) {
                    $legacy['border'] = array();
                    $legacy['border']['l'] = html2pdf_HTML2PDF::$_tables[$param['num']]['border'];
                    $legacy['border']['t'] = html2pdf_HTML2PDF::$_tables[$param['num']]['border'];
                    $legacy['border']['r'] = html2pdf_HTML2PDF::$_tables[$param['num']]['border'];
                    $legacy['border']['b'] = html2pdf_HTML2PDF::$_tables[$param['num']]['border'];
                }
            }
            ...
            if ($collapse) {
                if (!$this->_subPart) {
                    if (
                            (html2pdf_HTML2PDF::$_tables[$param['num']]['tr_curr'] > 1 && !html2pdf_HTML2PDF::$_tables[$param['num']]['new_page']) ||
                            (!$this->_isInThead && count(html2pdf_HTML2PDF::$_tables[$param['num']]['thead']['code']))
                    ) {
                        $this->parsingCss->value['border']['t'] = $this->parsingCss->readBorder('none');
                    }
                }

                if (html2pdf_HTML2PDF::$_tables[$param['num']]['td_curr'] > 0) {
                    if (!$return)
            ...
            $marge = array();
            $marge['t'] = $this->parsingCss->value['padding']['t'] + 0.5 * html2pdf_HTML2PDF::$_tables[$param['num']]['cellspacing'] + $this->parsingCss->value['border']['t']['width'];
            $marge['r'] = $this->parsingCss->value['padding']['r'] + 0.5 * html2pdf_HTML2PDF::$_tables[$param['num']]['cellspacing'] + $this->parsingCss->value['border']['r']['width'];
            $marge['b'] = $this->parsingCss->value['padding']['b'] + 0.5 * html2pdf_HTML2PDF::$_tables[$param['num']]['cellspacing'] + $this->parsingCss->value['border']['b']['width'];
            $marge['l'] = $this->parsingCss->value['padding']['l'] + 0.5 * html2pdf_HTML2PDF::$_tables[$param['num']]['cellspacing'] + $this->parsingCss->value['border']['l']['width'];

            // if we are in a sub HTML
            if ($this->_subPart) {
                // new position in the table
                html2pdf_HTML2PDF::$_tables[$param['num']]['td_curr'] ++;
                html2pdf_HTML2PDF::$_tables[$param['num']]['cases'][$y][$x] = array();
                html2pdf_HTML2PDF::$_tables[$param['num']]['cases'][$y][$x]['w'] = 0;
                html2pdf_HTML2PDF::$_tables[$param['num']]['cases'][$y][$x]['h'] = 0;
                html2pdf_HTML2PDF::$_tables[$param['num']]['cases'][$y][$x]['dw'] = 0;
                html2pdf_HTML2PDF::$_tables[$param['num']]['cases'][$y][$x]['colspan'] = $colspan;
                html2pdf_HTML2PDF::$_tables[$param['num']]['cases'][$y][$x]['rowspan'] = $rowspan;
                html2pdf_HTML2PDF::$_tables[$param['num']]['cases'][$y][$x]['Xr'] = html2pdf_HTML2PDF::$_tables[$param['num']]['corr_x'];
                html2pdf_HTML2PDF::$_tables[$param['num']]['cases'][$y][$x]['Yr'] = html2pdf_HTML2PDF::$_tables[$param['num']]['corr_y'];

                // prepare the mapping for rowspan and colspan
                for ($j = 0; $j < $rowspan; $j++) {
                    for ($i = 0; $i < $colspan; $i++) {
                        html2pdf_HTML2PDF::$_tables[$param['num']]['corr']
                                [html2pdf_HTML2PDF::$_tables[$param['num']]['corr_y'] + $j]
                                [html2pdf_HTML2PDF::$_tables[$param['num']]['corr_x'] + $i] = ($i + $j > 0) ? '' : array($x, $y, $colspan, $rowspan);
                    }
                }
                html2pdf_HTML2PDF::$_tables[$param['num']]['corr_x']+= $colspan;
                while (isset(html2pdf_HTML2PDF::$_tables[$param['num']]['corr'][html2pdf_HTML2PDF::$_tables[$param['num']]['corr_y']][html2pdf_HTML2PDF::$_tables[$param['num']]['corr_x']])) {
                    html2pdf_HTML2PDF::$_tables[$param['num']]['corr_x'] ++;
                }
                ...
                // new position in the table
                html2pdf_HTML2PDF::$_tables[$param['num']]['td_curr'] ++;
                html2pdf_HTML2PDF::$_tables[$param['num']]['td_x']+=html2pdf_HTML2PDF::$_tables[$param['num']]['cases'][$y][$x]['dw'];

                // borders and background of the TD
                $this->_drawRectangle(
                        html2pdf_HTML2PDF::$_tables[$param['num']]['td_x'], html2pdf_HTML2PDF::$_tables[$param['num']]['td_y'], html2pdf_HTML2PDF::$_tables[$param['num']]['cases'][$y][$x]['w'], html2pdf_HTML2PDF::$_tables[$param['num']]['cases'][$y][$x]['h'], $this->parsingCss->value['border'], $this->parsingCss->value['padding'], html2pdf_HTML2PDF::$_tables[$param['num']]['cellspacing'] * 0.5, $this->parsingCss->value['background']
                );

                $this->parsingCss->value['width'] = html2pdf_HTML2PDF::$_tables[$param['num']]['cases'][$y][$x]['w'] - $marge['l'] - $marge['r'];

                // marges = size of the TD
                $mL = html2pdf_HTML2PDF::$_tables[$param['num']]['td_x'] + $marge['l'];
                $mR = $this->pdf->getW() - $mL - $this->parsingCss->value['width'];
                $this->_saveMargin($mL, 0, $mR);

                // position of the content, from vertical-align
                $hCorr = html2pdf_HTML2PDF::$_tables[$param['num']]['cases'][$y][$x]['h'];
                $hReel = html2pdf_HTML2PDF::$_tables[$param['num']]['cases'][$y][$x]['real_h'];
                switch ($this->parsingCss->value['vertical-align']) {
                    case 'bottom':
                        $yCorr = $hCorr - $hReel;
                        break;
                ...
                $x = html2pdf_HTML2PDF::$_tables[$param['num']]['td_x'] + $marge['l'];
                $y = html2pdf_HTML2PDF::$_tables[$param['num']]['td_y'] + $marge['t'] + $yCorr;
                $this->pdf->setXY($x, $y);
                $this->_setNewPositionForNewLine();
            }
        ...
        protected function _tag_close_TD($param) {
            ...
            if ($this->_subPart) {
                ...
                if ($this->_testTdInOnepage && $this->_subHtml->pdf->getPage() > 1) {
                    throw new html2pdf_HTML2PDF_exception(7);
                }
                ...
                html2pdf_HTML2PDF::$_tables[$param['num']]['cases'][html2pdf_HTML2PDF::$_tables[$param['num']]['tr_curr'] - 1][html2pdf_HTML2PDF::$_tables[$param['num']]['td_curr'] - 1]['w'] = max(array($w0, $w2));
                html2pdf_HTML2PDF::$_tables[$param['num']]['cases'][html2pdf_HTML2PDF::$_tables[$param['num']]['tr_curr'] - 1][html2pdf_HTML2PDF::$_tables[$param['num']]['td_curr'] - 1]['h'] = max(array($h0, $h2));

                // real position of the content
                html2pdf_HTML2PDF::$_tables[$param['num']]['cases'][html2pdf_HTML2PDF::$_tables[$param['num']]['tr_curr'] - 1][html2pdf_HTML2PDF::$_tables[$param['num']]['td_curr'] - 1]['real_w'] = $w0;
                html2pdf_HTML2PDF::$_tables[$param['num']]['cases'][html2pdf_HTML2PDF::$_tables[$param['num']]['tr_curr'] - 1][html2pdf_HTML2PDF::$_tables[$param['num']]['td_curr'] - 1]['real_h'] = $h0;
                ...
                html2pdf_HTML2PDF::$_tables[$param['num']]['td_x']+=html2pdf_HTML2PDF::$_tables[$param['num']]['cases'][html2pdf_HTML2PDF::$_tables[$param['num']]['tr_curr'] - 1][html2pdf_HTML2PDF::$_tables[$param['num']]['td_curr'] - 1]['w'];
            }
