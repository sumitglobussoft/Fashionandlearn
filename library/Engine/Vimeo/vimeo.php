...
//        print_r($access_token); die;
   ...
   return $this->request(self::ACCESS_TOKEN_ENDPOINT, array(
     ...
     */
    public function clientCredentials($scope = array('public','upload','create','edit','delete','interact')) {
        if (is_array($scope)) {
            $scope = implode(' ', $scope);
        }
     ...
     * @return string
     */
    public function buildAuthorizationEndpoint($redirect_uri, $scope = array('public','upload','create','edit','delete','interact'), $state = 'uniquevimeo123') {
//        print_r($this->_client_id); die;
//        print_r($state); die;
        $query = array(
            "response_type" => 'code',
...
//       print_r(self::AUTH_ENDPOINT . '?' . http_build_query($query)); die;
