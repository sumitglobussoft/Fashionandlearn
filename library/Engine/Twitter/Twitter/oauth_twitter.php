...
class OAuth_Twitter {
	...
	function getFollowersByUserId($user_id){
	    ...
	    $client->setUri('https://api.twitter.com/1.1/followers/ids.json');
	...
	function getPublicUpdatesByHandle($screen_name){
	    $sxml = file_get_contents('https://api.twitter.com/1.1/statuses/user_timeline.json?screen_name='.$screen_name);
	...
	function getListUpdatesByUseridAndListId($user_id,$list_id){
	    //$sxml = file_get_contents('https://api.twitter.com/1.1/'.$user_id.'/lists/'.$list_id.'/statuses.json');
            $sxml = file_get_contents('https://api.twitter.com/1.1/lists/statuses.json?list_id='.$list_id.'&owner_id='.$user_id);
	...
	function getListUpdatesByUsernameAndListname($user_name,$list_name){
	    $sxml = file_get_contents('http://api.twitter.com/1/'.$user_id.'/lists/'.$list_name.'/statuses.json');
            //$sxml = file_get_contents('https://api.twitter.com/1.1/lists/statuses.json?list_id='.$list_id.'&owner_screen_name ='.$user_name);
	    return json_decode($sxml);
	}	
	...
	function followATweety($screen_name){
	    ...
	    $client->setUri('https://api.twitter.com/1.1/friendships/create.json ');
	    $client->setParameterGet('screen_name', $screen_name);
	    $client->setMethod(Zend_Http_Client::POST);
	    $response = $client->request();
	    return json_decode($response->getBody());
