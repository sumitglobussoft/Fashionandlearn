...
//Prevent any copy of this object
 ...
 if ($result) {
     ...
     public function getallproject(){
           $select = $this->select()
                  ->setIntegrityCheck(false)
                    ->from(array('p' => 'projects'),array("project_count" => "COUNT(p.project_id)"));
...
public  function getclassProjectsCount(){
        if (func_num_args() > 0) {
...
public function getTotalNoOFProjectsInFAshionlearn(){
  try {
                  $select = $this->select()
                            ->from(array('pr'=>'projects'))
                            ->setIntegrityCheck(false)
                            ->joinLeft(array('tc' => 'teachingclasses'),'pr.class_id = tc.class_id')
                            ->joinLeft(array('us' => 'users'),'pr.user_id = us.user_id')
