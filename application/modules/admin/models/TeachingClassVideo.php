...
//                print_r($result); die;
    ...
    public function getvideosIds() {
        try {
            $select = $this->select()
              ->setIntegrityCheck(false)
              ->from(array('tcv' => 'teachingclassvideo'), array('tcv.video_id','tcv.transcode_status'))
...
// dev: priyanka varanasi
    ...
    /* Developer:Rakesh Jha
      Desc : Get the total length of class
     */

    public function classLength() {
        if (func_num_args() > 0) {
            $class_id = func_get_arg(0);
            ...
            try {
                $select = $this->select()
                  ->setIntegrityCheck(false)
                  ->from(array('tcv' => 'teachingclassvideo'), array('SUM(tcv.video_duration)'))
                  ->where('tcv.class_id = ?', $class_id);
                $result = $this->getAdapter()->fetchRow($select);
            } catch (Exception $e) {
                return $e->getMessage();
