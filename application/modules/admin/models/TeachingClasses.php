...
WHERE teachingclasses.class_id = ' . $classid . '';
    ...
    public function getTeachersDetails() {
        ...
        $select = $this->select()
                ->from(array('tc' => 'teachingclasses'), array("class count" => "COUNT(*)", 'tc.user_id'))
                ->setIntegrityCheck(false)
...
//            die('dsadas');
   ...
   //dev: priyanka varanasi
   //desc: TO get all class based on publis status 0 taking projects count
   //date: 24/9/2015
    
    public function getClassesInPublishStatus(){
        
        
         try {
                $select = $this->select()
                        ->from(array('tc'=>'teachingclasses'),array('tc.class_id','tc.class_title'))
                        ->setIntegrityCheck(false)
                        ->joinLeft(array('pr' => 'projects'),'tc.class_id = pr.class_id', array("projectcount" => "COUNT(pr.project_id)"))
                        ->join(array('us' => 'users'),'tc.user_id = us.user_id',array("us.first_name",'us.last_name','us.email','us.user_id'))     
   ...
   //dev: priyanka varanasi
   //desc: TO get all class based on publis status 0 taking student count
   //date: 24/9/2015
    
    public function getClassesOfPublishStatus(){
         ...
         try {
                $select = $this->select()
                        ->from(array('tc'=>'teachingclasses'),array('tc.class_id','tc.class_title'))
                        ->setIntegrityCheck(false)
                        ->joinLeft(array('cer' => 'classenroll'),'tc.class_id = cer.class_id', array("studentcount" => "COUNT(cer.enroll_id)"))
                        ->join(array('us' => 'users'),'tc.user_id = us.user_id',array("us.first_name",'us.last_name','us.email','us.user_id'))     
                        ->where('tc.publish_status = ?',0)
                        ->group("tc.class_id");
    ...
    //dev: priyanka varanasi
   //desc: TO get percentage of satifaction based on reviews 
   //date: 24/9/2015
    
    
       public function getClassesReviewsOfPublishStatus(){
         ...
         try {
                $select = $this->select()
                        ->from(array('tc'=>'teachingclasses'),array('tc.class_id','tc.class_title'))
                        ->setIntegrityCheck(false)
                        ->joinLeft(array('crw' => 'classesreview'),'tc.class_id = crw.class_id', array("avgreviewspercentage" => "((1- AVG(crw.recommend_class))*100)"))
                        ->join(array('us' => 'users'),'tc.user_id = us.user_id',array("us.first_name",'us.last_name','us.email','us.user_id'))     
                        ->where('tc.publish_status = ?',0)
                        ->group("tc.class_id");
