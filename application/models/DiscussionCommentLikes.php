...
class Application_Model_DiscussionCommentLikes extends Zend_Db_Table_Abstract {

    private static $_instance = null;
    protected $_name = 'discussioncommentlikes';
    ...
    public static function getInstance() {
        if (!is_object(self::$_instance))  //or if( is_null(self::$_instance) ) or if( self::$_instance == null )
            self::$_instance = new Application_Model_DiscussionCommentLikes();
        return self::$_instance;
    }

    public function discusscommentlikes() {
        ...
        if (func_num_args() > 0) {
          ...
          $date = gmdate('Y-m-d h:i:s',  time());
            ...
            if ($result) {
                ...
                $data = array("user_id" => $userid, "class_id" => $classid, "discussion_id" => $discussionid, "comment_id" => $commentid, "like_status" => 0, "like_date" => $date);
    ...
    public function getdiscusscommentlikes() {

        if (func_num_args() > 0) {
            $commentid = func_get_arg(0);

            $select = $this->select()
                    ->from("discussioncommentlikes", array("num" => "COUNT(*)"))
                    ->where("comment_id = " . $commentid)
                    ->where("like_status = 0");
    ...
    public function getuserdiscusscommentlikes() {
