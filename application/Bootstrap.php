...
//        $objategoryModel = Application_Model_Category::getInstance();
//        $allCategories = $objategoryModel->getAllCategories();
...
//    }
    ...
    protected function preDispatch() {
     ...
     function _initTranslation() {
        ...
        $langPath = $_SERVER['DOCUMENT_ROOT'] . '/language/';

        // Create an instance of Zends gettext adapter
        $translate = new Zend_Translate_Adapter_Gettext($langPath . 'en.mo', 'en');
         $translate->addTranslation(
                array(
                    'content' => $langPath . 'pr.mo',
                    'locale' => 'pr'
                )
        );
                $translate->addTranslation(
                array(
                    'content' => $langPath . 'en.mo',
                    'locale' => 'en'
                )
        );
        ...
        if(!isset($langCookie)) {
            ...
            $translate->setLocale($langCookie);
        }
